...
export const EditInventoryItem = ({
  ...
  const filterSelected = inventory?.find(
    (item) => item._id === selectedItem?.id
  );
  ...
  const filterSubcategory = subcategory?.filter(
    (item) => item.category === chosen.category
  );
  const filterProduct = product?.filter((item) => {
    return (
      item.category === chosen.category &&
  ...
  return (
    <div>
      ...
      <SelectItem
        ...
        onChange={(e) => setChosen({ ...chosen, category: e.target.value })}
      >
        <option value="">Categoria</option>
        {category !== undefined &&
          category.map((item) => <option>{item.categoryName}</option>)}
      </SelectItem>
      <SelectItem
        value={chosen.subcategory}
        onChange={(e) => setChosen({ ...chosen, subcategory: e.target.value })}
      >
        <option value="">Subcategoria</option>
        {subcategory !== undefined &&
          filterSubcategory.map((item) => <option>{item.subcategory}</option>)}
      </SelectItem>
      <SelectItem
        value={chosen.product}
        onChange={(e) => setChosen({ ...chosen, product: e.target.value })}
      >
        <option value="">Produto</option>
        {product !== undefined &&
          filterProduct.map((item) => <option>{item.product}</option>)}
      </SelectItem>
      <InputItens
        type="text"
        value={chosen.size}
